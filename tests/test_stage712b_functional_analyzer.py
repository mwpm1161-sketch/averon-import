    ...
    rows = (
        {
            ...
            "cells": [
                ...
                {"source_cell_index": 2, "column_index": 2, "raw_text": "Ед.", "bbox": {"x": 120, "y": 0, "width": 40, "height": 20}},
                {"source_cell_index": 3, "column_index": 3, "raw_text": "Количество", "bbox": {"x": 160, "y": 0, "width": 40, "height": 20}},
            ],
        },
        {
            "source_row_index": 1,
        ...
        },
        {
            ...
            "cells": [
                {"source_cell_index": 8, "column_index": 0, "raw_text": "3", "bbox": {"x": 0, "y": 40, "width": 40, "height": 20}},
                {"source_cell_index": 9, "column_index": 2, "raw_text": "5", "bbox": {"x": 120, "y": 40, "width": 40, "height": 20}},
                {"source_cell_index": 10, "column_index": 3, "raw_text": "6", "bbox": {"x": 160, "y": 40, "width": 40, "height": 20}},
                {"source_cell_index": 11, "column_index": 4, "raw_text": "17", "bbox": {"x": 160, "y": 40, "width": 40, "height": 20}},
            ],
        },
    )
    return PhysicalEvidenceSnapshot(
        grid={"source": "offline-raster", "x_boundaries": [0, 40, 120, 160, 200], "y_boundaries": [0, 20, 40, 60]},
        selected_table_bounds={"x": 0, "y": 0, "width": 200, "height": 60},
        physical_rows=rows,
        spatial_words=(),
        assigned_word_refs={},
...
def test_snapshot_adapter_and_analyzer_are_wired_without_full_text_authority():
    ...
    assert assessments[2].state == RowRoleState.CONFIRMED
...
def test_p12_like_numbering_band_records_outlier_without_repairing_it():
    ...
    assert evidence["ocr_outliers"] == [{"column_index": 4, "observed": 17}]
...
def _numbering_graph(rows, *, header_rows=(0,)):
    ...
    table_rows = [
        {
            "source_row_index": 0,
            "bbox": {"x": 0, "y": 0, "width": 240, "height": 20},
            "cells": [
                {"source_cell_index": index, "column_index": index, "raw_text": text,
                 "bbox": {"x": index * 60, "y": 0, "width": 60, "height": 20}}
                for index, text in enumerate(("Позиция", "Наименование", "Ед.", "Количество"))
            ],
        }
    ]
        ...
        table_rows.append(
            {
                ...
                "bbox": {"x": 0, "y": row_index * 20, "width": 240, "height": 20},
                "cells": [
    ...
    snapshot = PhysicalEvidenceSnapshot(
        grid={"source": "offline-raster", "x_boundaries": [0, 60, 120, 180, 240],
              "y_boundaries": [0, 20, 40, 60, 80, 100]},
        selected_table_bounds={"x": 0, "y": 0, "width": 240, "height": 100},
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("N2 sparse ordinals", [(1, ("1", "", "3", "5"))], RowRole.SERVICE, RowRoleState.CONFIRMED),
        ("N3 multi-digit OCR outlier", [(1, ("3", "5", "6", "17"))], RowRole.SERVICE, RowRoleState.CONFIRMED),
        ("N4 missing digits", [(1, ("1", "3", "4"))], RowRole.SERVICE, RowRoleState.CONFIRMED),
